% % Q1(1）
% Kp = 2.5;
% Ki = .5;
% Kd = 0;
% 
% % integral error
% ei = 0;
% 
% % previous error (for derivative)
% e_prev = 0;
% 
% %initial voltage
% u=[4;4];

% % Q1(2）
% u = [4;-4];

% % Q2
% % kid factors
% Kp = 10;
% Ki = 0.1;
% Kd = 0;
% 
% % integral error
% ei = 0;
% 
% % previous error (for derivative)
% e_prev = 0;
% 
% %initial voltage
% u=[4;4];
% 
% % previous tick counts
% y1_p=0;
% y2_p=0;
% 
% lw = 0.250;
% Rw = 0.100;
% x_e = 0;
% y_e = 0;
% theta = 0;
% 
% % label
% label = 0;

% Q3
% the list containing all reference points
lst = [1.375,0;1.375,-1;2-0.625*cos(pi/16),-1-0.625*sin(pi/16);2-0.625*cos(2*pi/16),-1-0.625*sin(2*pi/16);2-0.625*cos(3*pi/16),-1-0.625*sin(3*pi/16);...
2-0.625*cos(4*pi/16),-1-0.625*sin(4*pi/16);2-0.625*cos(5*pi/16),-1-0.625*sin(5*pi/16);2-0.625*cos(6*pi/16),-1-0.625*sin(6*pi/16);2-0.625*cos(7*pi/16),-1-0.625*sin(7*pi/16);...
2,-1.625;3,-1.625;3+0.625*sin(pi/16),-1-0.625*cos(pi/16);3+0.625*sin(2*pi/16),-1-0.625*cos(2*pi/16);3+0.625*sin(3*pi/16),-1-0.625*cos(3*pi/16);...
3+0.625*sin(4*pi/16),-1-0.625*cos(4*pi/16);3+0.625*sin(5*pi/16),-1-0.625*cos(5*pi/16);3+0.625*sin(6*pi/16),-1-0.625*cos(6*pi/16);3+0.625*sin(7*pi/16),-1-0.625*cos(7*pi/16);...
3.625,-1;3.625,1;3+0.625*cos(pi/16),1+0.625*sin(pi/16);3+0.625*cos(2*pi/16),1+0.625*sin(2*pi/16);3+0.625*cos(3*pi/16),1+0.625*sin(3*pi/16);...
3+0.625*cos(4*pi/16),1+0.625*sin(4*pi/16);3+0.625*cos(5*pi/16),1+0.625*sin(5*pi/16);3+0.625*cos(6*pi/16),1+0.625*sin(6*pi/16);3+0.625*cos(7*pi/16),1+0.625*sin(7*pi/16);...
3,1.625;2,1.625;2-0.625*sin(pi/16),1+0.625*cos(pi/16);2-0.625*sin(2*pi/16),1+0.625*cos(2*pi/16);2-0.625*sin(3*pi/16),1+0.625*cos(3*pi/16);...
2-0.625*sin(4*pi/16),1+0.625*cos(4*pi/16);2-0.625*sin(5*pi/16),1+0.625*cos(5*pi/16);2-0.625*sin(6*pi/16),1+0.625*cos(6*pi/16);2-0.625*sin(7*pi/16),1+0.625*cos(7*pi/16);...
1.375,1;1.375,-1;2-0.625*cos(pi/16),-1-0.625*sin(pi/16);2-0.625*cos(2*pi/16),-1-0.625*sin(2*pi/16);2-0.625*cos(3*pi/16),-1-0.625*sin(3*pi/16);...
2-0.625*cos(4*pi/16),-1-0.625*sin(4*pi/16);2-0.625*cos(5*pi/16),-1-0.625*sin(5*pi/16);2-0.625*cos(6*pi/16),-1-0.625*sin(6*pi/16);2-0.625*cos(7*pi/16),-1-0.625*sin(7*pi/16);...
2,-1.625;3,-1.625;3+0.625*sin(pi/16),-1-0.625*cos(pi/16);3+0.625*sin(2*pi/16),-1-0.625*cos(2*pi/16);3+0.625*sin(3*pi/16),-1-0.625*cos(3*pi/16);...
3+0.625*sin(4*pi/16),-1-0.625*cos(4*pi/16);3+0.625*sin(5*pi/16),-1-0.625*cos(5*pi/16);3+0.625*sin(6*pi/16),-1-0.625*cos(6*pi/16);3+0.625*sin(7*pi/16),-1-0.625*cos(7*pi/16);...
3.625,-1;3.625,1;3+0.625*cos(pi/16),1+0.625*sin(pi/16);3+0.625*cos(2*pi/16),1+0.625*sin(2*pi/16);3+0.625*cos(3*pi/16),1+0.625*sin(3*pi/16);...
3+0.625*cos(4*pi/16),1+0.625*sin(4*pi/16);3+0.625*cos(5*pi/16),1+0.625*sin(5*pi/16);3+0.625*cos(6*pi/16),1+0.625*sin(6*pi/16);3+0.625*cos(7*pi/16),1+0.625*sin(7*pi/16);...
3,1.625;2,1.625;2-0.625*sin(pi/16),1+0.625*cos(pi/16);2-0.625*sin(2*pi/16),1+0.625*cos(2*pi/16);2-0.625*sin(3*pi/16),1+0.625*cos(3*pi/16);...
2-0.625*sin(4*pi/16),1+0.625*cos(4*pi/16);2-0.625*sin(5*pi/16),1+0.625*cos(5*pi/16);2-0.625*sin(6*pi/16),1+0.625*cos(6*pi/16);2-0.625*sin(7*pi/16),1+0.625*cos(7*pi/16);...
1.375,1;1.375,0;0,0;0,0];
label = 1;
x_d = 1.375;
y_d = 0;

% kid factors
Kp = 10;
Ki = 0.1;
Kd = 0;

% integral error
ei = 0;

% previous error (for derivative)
e_prev = 0;

%initial voltage
u=[0;0];

% previous tick counts
y1_p=0;
y2_p=0;

lw = 0.250;
Rw = 0.100;
x_e = 0;
y_e = 0;
theta = 0;

